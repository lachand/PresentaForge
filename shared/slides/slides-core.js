...
.sl-title-content { width: min(100%, 1040px); margin-block: auto; display: flex; flex-direction: column; align-items: center; }
...
.sl-chapter-content { width: min(100%, 1040px); margin-block: auto; display: flex; flex-direction: column; align-items: center; }
                ...
                content = `<div style="width:100%;height:100%;background:${tone.softBg};border:1px solid ${tone.border};border-radius:10px;padding:0.75rem 0.85rem;box-sizing:border-box;display:flex;flex-direction:column;gap:0.5rem;">
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.55rem;min-height:0;flex:1;">
                        <div style="display:flex;flex-direction:column;min-height:0;border:1px solid color-mix(in srgb,var(--sl-danger,#ef4444) 40%,var(--sl-border,#2d3347));border-radius:8px;overflow:hidden;background:var(--sl-code-bg,#0d1117);">
                            ...
                            <pre style="margin:0;padding:8px 10px;flex:1;overflow:auto;font-size:${Math.round(base * 0.78)}px;line-height:${codeLineHeight};font-family:var(--sl-font-mono,monospace);color:var(--sl-code-text,#f8fafc);white-space:pre-wrap;"><code class="language-${SlidesRenderer.esc(lang)}">${SlidesRenderer.esc(mistake)}</code></pre>
                        </div>
                        <div style="display:flex;flex-direction:column;min-height:0;border:1px solid color-mix(in srgb,var(--sl-success,#22c55e) 40%,var(--sl-border,#2d3347));border-radius:8px;overflow:hidden;background:var(--sl-code-bg,#0d1117);">
                            ...
                            <pre style="margin:0;padding:8px 10px;flex:1;overflow:auto;font-size:${Math.round(base * 0.78)}px;line-height:${codeLineHeight};font-family:var(--sl-font-mono,monospace);color:var(--sl-code-text,#f8fafc);white-space:pre-wrap;"><code class="language-${SlidesRenderer.esc(lang)}">${SlidesRenderer.esc(fix)}</code></pre>
                        </div>
                    </div>
                </div>`;
                break;
            ...
            case 'code-example': {
                ...
                if (mode === 'live') {
                    widget = `<div style="width:100%;height:100%;display:flex;flex-direction:column;min-height:0;">
                        ...
                        <pre style="margin:0;padding:8px 10px;font-size:${Math.round(base * 0.82)}px;line-height:${codeLineHeight};font-family:var(--sl-font-mono);color:var(--sl-code-text,#f8fafc);background:var(--sl-code-bg,#0d1117);white-space:pre;overflow:auto;flex:1;"><code class="language-${SlidesRenderer.esc(lang)}">${SlidesRenderer.esc(code)}</code></pre>
                    </div>`;
            ...
            case 'smartart': {
                ...
                if (variant === 'process') {
                    ...
                    const rows = items.map((item, i) => {
                        ...
                        return `<div style="width:${w}%;padding:10px;border-radius:6px;text-align:center;color:var(--sl-text);background:color-mix(in srgb,${color} ${20+60*(count-i)/count}%,var(--sl-slide-bg));border:1px solid ${color};margin:0 auto;">${SlidesRenderer.esc(item)}</div>`;
                    }).join('');
            ...
            case 'quiz-live': {
                ...
                const optsHtml = opts.map((o, i) =>
                    `<div class="sl-quizlive-option" data-idx="${i}" style="padding:10px 16px;border:2px solid var(--sl-border,#2d3347);border-radius:8px;cursor:pointer;display:flex;align-items:center;gap:10px;transition:all 0.2s;pointer-events:auto;">
                        ...
                        <span style="color:var(--sl-text,#cbd5e1);font-size:var(--sl-quiz-option-size,1rem);line-height:1.4;">${SlidesRenderer.esc(o)}</span>
                    </div>`
                ).join('');
