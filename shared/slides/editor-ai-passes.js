...
function _buildGeminiPassPrompt(pass, { brief, tuning, pipeline, previous, promptTemplate, refinePrompt = '', currentDraft = '' }) {
    ...
    if (pass === 1) {
        return [
            ...
            'Schéma JSON strict:',
            '{"pass":1,"title":"...","learningObjectives":["..."],"plan":[{"order":1,"title":"...","type":"title|chapter|bullets|code|split|definition|comparison|image|quote|blank|canvas","objective":"...","keyPoints":["..."],"level":1}],"risks":["..."]}',
            'Consignes: privilégie des slides compatibles avec des composants natifs riches (canvas + smartart/diagramme/card/definition/highlight/quiz-live/widget), sans dépendance image lourde. Les QCM sont des éléments canvas quiz-live (pas une slide "quiz").',
    ...
    if (pass === 2) {
        return [
            ...
            'Réponds uniquement avec un JSON valide, sans markdown.',
            'Schéma JSON strict:',
            '{"pass":2,"illustrations":[{"slide":1,"slideTitle":"...","visualType":"icon|infographic|diagram|photo|schema","intent":"...","placement":"left|right|full","keywords":["..."],"assetHint":"..."}],"componentPlan":[{"slide":1,"componentType":"quiz-live|mcq-single|mcq-multi|poll-likert|exit-ticket|highlight|smartart|diagramme|card|definition|table|mermaid|widget|timeline-vertical|swot-grid|list|quote|shape","intent":"...","placement":"left|right|full","priority":"high|normal|low","payloadHint":"..."}]}',
    ...
    if (pass === 3) {
        ...
        return [
            ...
            '- Préfère les composants natifs visuels (smartart, diagramme, card, definition, highlight, widget, quiz-live, algo-stepper). Code source = `highlight` (jamais `code`/`code-example`). QCM = `quiz-live` (jamais slide `quiz`), avec `data.explanation`.',
            '- POIDS MÉDIA: images en SVG base64 léger ou placeholder ; jamais de data URL base64 > 250 Ko ; deck total < 1,5 Mo (au-delà: sauvegarde Firebase et synchro temps réel cassées).',
            'Règles d’équilibre obligatoires:',
            ...adaptiveRules,
            quizTarget > 0
    ...
    if (pass === 4) {
        return [
            ...
            '- Remplacer tout `code`/`code-example`/`terminal-session` par `highlight` ; toute slide `quiz` par un élément canvas `quiz-live`.',
            '- Ajouter `data.explanation` à chaque `quiz-live`/`mcq-single`/`mcq-multi` qui n’en a pas.',
            '- POIDS MÉDIA: aucune image base64 > 250 Ko ; deck total < 1,5 Mo.',
            baseContext,
            `Brief utilisateur: ${brief}`,
            `JSON PASS 3:\n${previous.pass3 || ''}`,
            relanceBlock,
    ...
    return [
        ...
        'Checklist finale: JSON parseable, schemaVersion=2, types supportés, pas de [object Object], formats sensibles conformes, équilibre visuel respecté.',
        '- Aucun élément `code`/`code-example`/`terminal-session`, aucune slide `quiz`.',
        '- `data.explanation` présent sur chaque `quiz-live`/`mcq-single`/`mcq-multi`.',
        '- POIDS MÉDIA conforme: aucune image base64 > 250 Ko, deck total < 1,5 Mo.',
        quizTarget > 0
